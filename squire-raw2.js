...
var canObserveMutations = typeof MutationObserver !== 'undefined' && !isIOS;
...
function TreeWalker ( root, nodeType, filter ) {
	console.log("TreeWalker.constructor");
...
TreeWalker.prototype.previousNode = function () {
	console.log("Treewalker.previousNode");
    var current = this.currentNode,
        root = this.root,
        nodeType = this.nodeType,
        filter = this.filter,
...
TreeWalker.prototype.previousPONode = function () {
	console.log("Treewalker.previousPONode");
    var current = this.currentNode,
        root = this.root,
        nodeType = this.nodeType,
        filter = this.filter,
...
function hasTagAttributes ( node, tag, attributes ) {
	console.log("Squire.hasTagAttributes");
...
function isOrContains ( parent, node ) {
	console.log("Squire.isOrContains");
    while ( node ) {
        if ( node === parent ) {
            return true;
        }
...
function replaceWith ( node, node2 ) {
	console.log("Squire.replaceWith");
    var parent = node.parentNode;
    if ( parent ) {
...
function createElement ( doc, tag, props, children ) {
	console.log("Squire.createElement");
    ...
    if ( fixer ) {
        try {
            ...
            getSquireInstance( doc ).didError({
                name: 'Squire: fixCursor â€“Â ' + error,
...
function fixContainer ( container, root ) {
	console.log("Squire.fixContainer");
...
function mergeInlines ( node, range ) {
	console.log("Squire.mergeInlines");
    if ( node.nodeType !== ELEMENT_NODE ) {
        return;
    }
...
function mergeWithBlock ( block, next, range ) {
	console.log("Squire.mergeWithBlock");
    var container = next,
        last, offset, _range;
...
function mergeContainers ( node, root ) {
	console.log("Squire.mergeContainers");
...
var getNodeBefore = function ( node, offset ) {
	console.log("Squire.getNodeBefore");
    var children = node.childNodes;
...
var getNodeAfter = function ( node, offset ) {
	console.log("Squire.getNodeAfter");
    if ( node.nodeType === ELEMENT_NODE ) {
        var children = node.childNodes;
...
var insertNodeInRange = function ( range, node ) {
	console.log("Squire.insertNodeInRange");
    // Insert at start.
...
var extractContentsOfRange = function ( range, common, root ) {
	console.log("Squire.extractContentsOfRange");
    var startContainer = range.startContainer,
        startOffset = range.startOffset,
        endContainer = range.endContainer,
        endOffset = range.endOffset;
...
var deleteContentsOfRange = function ( range, root ) {
	console.log("Squire.deleteContentsOfRange");
...
var insertTreeFragmentIntoRange = function ( range, frag, root ) {
	console.log("Squire.insertTreeFragmentIntoRange");
...
var isNodeContainedInRange = function ( range, node, partial ) {
	console.log("Squire.isNodeContainedInRange");
...
var moveRangeBoundariesDownTree = function ( range ) {
	console.log("Squire.moveRangeBoundariesDownTree");
    var startContainer = range.startContainer,
        startOffset = range.startOffset,
        endContainer = range.endContainer,
        endOffset = range.endOffset,
...
var moveRangeBoundariesUpTree = function ( range, common ) {
	console.log("Squire.moveRangeBoundariesUpTree");
    var startContainer = range.startContainer,
        startOffset = range.startOffset,
        endContainer = range.endContainer,
        endOffset = range.endOffset,
...
var getStartBlockOfRange = function ( range, root ) {
	console.log("Squire.getStartBlockOfRange");
...
var getEndBlockOfRange = function ( range, root ) {
	console.log("Squire.getEndBlockOfRange");
...
var rangeDoesStartAtBlockBoundary = function ( range, root ) {
	console.log("Squire.rangeDoesStartAtBlockBoundary");
    var startContainer = range.startContainer;
...
var rangeDoesEndAtBlockBoundary = function ( range, root ) {
	console.log("Squire.rangeDoesEndAtBlockBoundary");
...
var expandRangeToBlockBoundaries = function ( range, root ) {
	console.log("Squire.expandRangeToBlockBoundaries");
...
var mapKeyToFormat = function ( tag, remove ) {
	console.log("Squire.mapKeyToFormat");
    remove = remove || null;
    return function ( self, event ) {
        event.preventDefault();
...
var afterDelete = function ( self, range ) {
	console.log("Squire.afterDelete");
    try {
...
var keyHandlers = {
    enter: function ( self, event, range ) {
        ...
        while (nodeAfterSplit.nodeType === ELEMENT_NODE) {
            nodeAfterSplit.style.textAlign = block.style.textAlign;
            nodeAfterSplit.style.fontSize = block.style.fontSize;
...
keyHandlers[ctrlKey + 'c'] = mapKeyTo('copy');
keyHandlers[ctrlKey + 'x'] = mapKeyTo('cut');
keyHandlers[ctrlKey + 'v'] = mapKeyTo('paste');
keyHandlers[ctrlKey + 'shift-v'] = mapKeyTo('paste');
...
var replaceWithTag = function ( tag ) {
	console.log("Squire.replaceWithTag");
...
var replaceStyles = function ( node, parent ) {
	console.log("Squire.replaceStyles");
...
var removeEmptyInlines = function removeEmptyInlines ( node ) {
	console.log("Squire.removeEmptyInlines");
    var children = node.childNodes,
        l = children.length,
        child;
    while ( l-- ) {
...
var notWSTextNode = function ( node ) {
	console.log("Squire.notWSTextNode");
    return node.nodeType === ELEMENT_NODE ?
        node.nodeName === 'BR' :
        notWS.test( node.data );
};
var isLineBreak = function ( br ) {
	console.log("Squire.isLineBreak");
...
function getSquireInstance ( doc ) {
	console.log("Squire.getSquireInstance");
...
function mergeObjects ( base, extras ) {
	console.log("Squire.mergeObjects");
    var prop, value;
    if ( !base ) {
        base = {};
    }
...
function Squire ( root, config ) {
    ...
    //this.addEventListener("selectionchange", enableRestoreSelection);
    ...
    var that = this;
    this.addEventListener('squireInstanceReady', function () { that._isReady = true; });
...
proto.doCut = onCut;
proto.doGetNearest = function (node, root, tag, attributes) {
	console.log("Squire.doGetNearest");
    return getNearest(node, root, tag, attributes);
};

proto.setConfig = function ( config ) {
	console.log("Squire.setConfig");
    config = mergeObjects({
        blockTag: 'DIV',
...
proto.createDefaultBlock = function ( children ) {
	console.log("Squire.createDefaultBlock");
...
proto.didError = function ( error ) {
	console.log("Squire.didError");
    console.log( error );
};

proto.getDocument = function () {
	console.log("Squire.getDocument");
    return this._doc;
};
proto.getRoot = function () {
	console.log("Squire.getRoot");
    return this._root;
};

proto.modifyDocument = function ( modificationCallback ) {
	console.log("Squire.modifyDocument");
...
// editor code.
var customEvents = {
    pathChange: 1, select: 1, input: 1, undoStateChange: 1, squireInstanceReady: 1
};

proto.fireEvent = function ( type, event ) {
	console.log("Squire.fireEvent");
	console.log(type);
	console.log(event);
...
proto.handleEvent = function ( event ) {
	console.log("Squire.handleEvent");
    this.fireEvent( event.type, event );
};

proto.addEventListener = function ( type, fn ) {
	console.log("Squire.addEventListener");
    var handlers = this._events[ type ];
...
proto.getCursorPosition = function ( range ) {
	console.log("Squire.getCursorPosition");
...
proto._moveCursorTo = function ( toStart ) {
	console.log("Squire._moveCursorTo");
    var root = this._root,
...
proto.moveCursorToStart = function () {
	console.log("Squire.moveCursorToStart");
    return this._moveCursorTo( true );
};
proto.moveCursorToEnd = function () {
	console.log("Squire.moveCursorToEnd");
    return this._moveCursorTo( false );
};

var getWindowSelection = function ( self ) {
	console.log("Squire.getWindowSelection");
    return self._win.getSelection() || null;
};

proto.setSelection = function ( range ) {
	console.log("Squire.setSelection");
    if (range) {
        ...
        if (!this._isFocused) {
            ...
            if (isIOS) {
                ...
                // HACK: iOS mobile device bug: when selecting text and performing an action, then performing another action on the same current selection, 
                // the selection will lose highlighting. However, the action will still be performed. This happens because getWindowSelection comes back as an object
                // but with the incorrect range (usually null or empty properties). This does not happen if you set a timeout before calling getWindowSelection. 
                // Yes, this is a hack (a very ugly one)... 
                //setTimeout(function () {
                //    var sel = getWindowSelection(that);
                //    if (sel) {
                //        sel.removeAllRanges();
                //        sel.addRange(range);
                //    }
...
proto.getSelection = function () {
	console.log("Squire.getSelection");
    var sel = getWindowSelection( this );
    var root = this._root;
...
function enableRestoreSelection () {
	console.log("Squire.enableRestoreSelection");
    this._restoreSelection = true;
}
function disableRestoreSelection () {
	console.log("Squire.disableRestoreSelection");
    this._restoreSelection = false;
}
function restoreSelection () {
	console.log("Squire.restoreSelection");
...
proto.getSelectedText = function () {
	console.log("Squire.getSelectedText");
    var range = this.getSelection(),
        walker = new TreeWalker(
...
proto._didAddZWS = function () {
	console.log("Squire._didAddZWS");
    this._hasZWS = true;
};
proto._removeZWS = function () {
	console.log("Squire._removeZWS");
    if ( !this._hasZWS ) {
        return;
    }
...
proto._updatePath = function ( range, force ) {
	console.log("Squire._updatePath");
...
proto._updatePathOnEvent = function () {
	console.log("Squire._updatePathOnEvent");
...
proto.focus = function () {
	console.log("Squire.focus");
    this._root.focus();
    return this;
};

proto.blur = function () {
	console.log("Squire.blur");
    this._root.blur();
    return this;
};
...
proto._saveRangeToBookmark = function ( range ) {
	console.log("Squire._saveRangeToBookmark");
...
proto._getRangeAndRemoveBookmark = function ( range ) {
	console.log("Squire._getRangeAndRemoveBookmark");
    var root = this._root,
...
proto._keyUpDetectChange = function ( event ) {
	console.log("Squire._keyUpDetectChange");
    var code = event.keyCode;
    ...
    if ( !event.ctrlKey && !event.metaKey && !event.altKey &&
        ...
        if(event.ctrlKey){
            ...
            event.preventDefault();
            this._ignoreChange = true;
            setTimeout(function(){this._ignoreChange = false}.bind(this),100)
        }
    }
};

proto._docWasChanged = function () {
	console.log("Squire._docWasChanged");
    var changes = arguments[0];
    var observer = arguments[1];
    if(changes.length == 2){
        if(Array.prototype.every.call(changes[0].addedNodes,function(obj,index){
            return !!~Array.prototype.indexOf.call(changes[1].removedNodes,obj);
        })){

            console.log("Redundant change detected");
            observer.disconnect(); // disable mutation observer to avoid infinite loop?
            observer.takeRecords();
            console.log(arguments);
            // var t = this;
            // setTimeout(function(){observer.observe(t._root, {
            //     childList: true,
            //     attributes: true,
            //     characterData: true,
            //     subtree: true
            // })},100);
            canObserveMutations = false;
            return;
        }
    }
...
proto._recordUndoState = function ( range ) {
	console.log("Squire._recordUndoState");
...
proto.saveUndoState = function ( range ) {
	console.log("Squire.saveUndoState");
    if ( range === undefined ) {
        range = this.getSelection();
    }
    if ( !this._isInUndoState ) {
...
proto.undo = function () {
    ...
    if ( this._undoIndex !== 0 || !this._isInUndoState ) {
        ...
        this.fireEvent('input', { isUndo: true} );
    }
    return this;
};

proto.redo = function () {
	console.log("Squire.redo");
...
proto.getFontInfo = function ( range ) {
	console.log("Squire.getFontInfo");
    var fontInfo = {
        color: undefined,
...
proto._removeFormat = function ( tag, attributes, range, partial ) {
	console.log("Squire._removeFormat");
    // Add bookmark
    this._saveRangeToBookmark( range );
...
proto.changeFormat = function ( add, remove, range, partial ) {
	console.log("Squire.changeFormat");
...
proto.forEachBlock = function ( fn, mutates, range ) {
	console.log("Squire.forEachBlock");
    if ( !range && !( range = this.getSelection() ) ) {
        return this;
    }
...
proto.modifyBlocks = function ( modify, range ) {
	console.log("Squire.modifyBlocks");
    if ( !range && !( range = this.getSelection() ) ) {
        return this;
    }
...
var increaseBlockQuoteLevel = function ( frag ) {
	console.log("Squire.increaseBlockQuoteLevel");
...
var decreaseBlockQuoteLevel = function ( frag ) {
	console.log("Squire.decreaseBlockQuoteLevel");
    var root = this._root;
...
var removeBlockQuote = function (/* frag */) {
	console.log("Squire.removeBlockQuote");
...
var makeUnorderedList = function ( frag ) {
	console.log("Squire.makeUnorderedList");
    makeList( this, frag, 'UL' );
    return frag;
};

var makeOrderedList = function ( frag ) {
	console.log("Squire.makeOrderedList");
    makeList( this, frag, 'OL' );
    return frag;
};

var removeList = function ( frag ) {
	console.log("Squire.removeList");
...
var increaseListLevel = function (frag) {
	console.log("Squire.increaseListLevel");
...
var decreaseListLevel = function ( frag ) {
	console.log("Squire.decreaseListLevel");
    var root = this._root;
    var items = frag.querySelectorAll( 'LI' );
...
proto._ensureBottomLine = function () {
	console.log("Squire._ensureBottomLine");
    var root = this._root;
...
proto.setKeyHandler = function ( key, fn ) {
	console.log("Squire.setKeyHandler");
...
proto._getHTML = function () {
	console.log("Squire._getHTML");
    return this._root.innerHTML;
};

proto._setHTML = function ( html ) {
	console.log("Squire._setHTML");
    var root = this._root;
    var node = root;
    node.innerHTML = html;
    do {
...
proto.setHTML = function ( html ) {
    ...
    // Parse HTML into DOM tree
    if (html != null) { //IE/Edge take the very literal approach of adding the text "null" to the html
        div.innerHTML = html;
    }
...
proto.insertElement = function ( el, range ) {
	console.log("Squire.insertElement");
    if ( !range ) { range = this.getSelection(); }
    range.collapse( true );
    if ( isInline( el ) ) {
        insertNodeInRange( range, el );
...
proto.insertImage = function ( src, attributes ) {
	console.log("Squire.insertImage");
...
var escapeHTMLFragement = function ( text ) {
	console.log("Squire.escapeHTMLFragement");
...
proto.insertPlainText = function ( plainText, isPaste ) {
	console.log("Squire.insertPlainText");
...
proto.setFontFace = function ( name ) {
	console.log("Squire.setFontFace");
    this.changeFormat({
        tag: 'SPAN',
        attributes: {
            'class': 'font',
...
proto.setFontSize = function ( size ) {
	console.log("Squire.setFontSize");
    this.changeFormat({
        tag: 'SPAN',
        attributes: {
            'class': 'size',
...
proto.setTextColour = function ( colour ) {
	console.log("Squire.setTextColour");
    this.changeFormat({
        tag: 'SPAN',
        attributes: {
            'class': 'colour',
...
proto.setHighlightColour = function ( colour ) {
	console.log("Squire.setHighlightColour");
    this.changeFormat({
        tag: 'SPAN',
        attributes: {
            'class': 'highlight',
...
proto.setTextAlignment = function (alignment) {
	console.log("Squire.setTextAlignment");
    //var root = this._root;
    //var parent;
...
proto.setTextDirection = function ( direction ) {
	console.log("Squire.setTextDirection");
    this.forEachBlock( function ( block ) {
        block.dir = direction;
    }, true );
    return this.focus();
...
function removeFormatting ( self, root, clean ) {
	console.log("Squire.removeFormatting");
    var node, next;
...
proto.removeAllFormatting = function ( range ) {
	console.log("Squire.removeAllFormatting");
